...
> 2026-03-15 update:
> - Frontend global filtering is now presented in a shared top-level bar instead of being split between sidebar and per-page chip bars.
> - Shared global filters are: `selectedClientId`, `dateFrom/dateTo` (or period preset), `campaignType`, `status`.
> - View-specific controls remain local and should not be added to the global filter bar.
...
- `POST /sync/trigger?client_id={id}&days=30` → trigger sync
