...
export default function AssetGroupsSection({ data }) {
    if (!data?.asset_groups?.length) return <div style={{ padding: '0 16px 16px', fontSize: 12, color: C.w40 }}>Brak grup zasobow PMax.</div>
    return (
                    ...
                    {data.asset_groups.map((ag, i) => {
                        ...
                        return (
                            <tr key={i} style={{ borderBottom: `1px solid ${C.w04}` }}>
                                <td style={{ ...TD, fontFamily: 'inherit', fontWeight: 500, color: C.textPrimary }}>{ag.name}</td>
                                <td style={{ ...TD, textAlign: 'right' }}>
                                    <span style={{ padding: '2px 8px', borderRadius: 999, fontSize: 10, fontWeight: 600, background: `${AD_STRENGTH_COLOR[ag.ad_strength] || C.w10}22`, color: AD_STRENGTH_COLOR[ag.ad_strength] || C.w50 }}>{ag.ad_strength || '—'}</span>
                                </td>
