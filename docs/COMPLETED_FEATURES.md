# Completed Features â€” DO NOT MODIFY UNLESS ASKED
...
- CRITICAL: Do NOT add `segments.keyword.info.*` to campaign_search_term_view queries â€” it filters out PMax data
...
- Campaigns list: snapshot data â€” NO date filtering
...
## AppContext â€” Centralized Client State
...
- Model `KeywordDaily`: keyword_id + date â†’ clicks, impressions, cost_micros, conversions, conversion_value_micros, avg_cpc_micros
- Router `keywords_ads.py`: two paths â€” daily aggregation (with date_from/date_to) vs snapshot (without dates)
...
- `SearchOptimization.jsx` â€” 6 collapsible analysis sections
