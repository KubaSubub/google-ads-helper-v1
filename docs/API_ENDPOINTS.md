...
- `GET /ad_groups/{ad_group_id}/ads` — list ads in a single ad group with snapshot metrics + RSA preview (ad_type, status, ad_strength, approval_status, headline_1/2, headlines/descriptions counts, final_url, clicks, impressions, cost, conversions, CTR); used by Campaigns page expand-row drill-down
- `GET /ads/?client_id=X&campaign_id=&ad_group_id=&status=&sort_by=cost&sort_order=desc&page=1&page_size=50`
- `GET /ads/{ad_id}` — full ad detail with RSA assets breakdown (headlines/descriptions with `pinned_position` + `performance_label`), ad metadata (ad_type, status, ad_strength, approval_status, final_url, long_headline, business_name), snapshot metrics, and `comparison.avg` + `comparison.diff_pct` vs sibling ads in the same group; powers Ad Detail Drawer on Campaigns page (`ads.py` router, separate from `/ads/` list in `keywords_ads.py`)
